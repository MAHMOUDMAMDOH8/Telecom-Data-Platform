...
1. Create DAG files in the Airflow `dags/` directory
2. Define tasks for:
   - Loading dimensions
   - Loading facts
   - Data quality checks
...
dag = DAG('telecom_data_pipeline', ...)

load_dims = SparkSubmitOperator(
    task_id='load_dimensions',
    application='/opt/spark_shared/Modeling/Dims/dimension.py',
    ...
)

load_facts = SparkSubmitOperator(
    task_id='load_facts',
    application='/opt/spark_shared/Modeling/Fact/fact.py',
    ...
)
```
...
### Load Dimensions
```bash
docker exec -it <spark-container> spark-submit \
  --master local[*] \
...
docker exec -it <spark-container> spark-submit \
  --master local[*] \
  --packages org.apache.iceberg:iceberg-spark-runtime-3.3_2.12:1.3.1 \
  /opt/spark_shared/Modeling/Fact/fact.py calls
```
...
### Spark Configuration
Key Spark configurations for Iceberg and S3:
- `spark.sql.catalog.my_catalog.type=hadoop`
- `spark.sql.catalog.my_catalog.warehouse=s3a://telecomlakehouse/iceberg`
...
For more details, explore the code in the `spark_shared/` directory and refer to individual module documentation.
